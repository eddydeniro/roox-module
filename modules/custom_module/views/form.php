...
<?php echo form_tag('configuration_form', url_for("$plugin_name/custom_module/code",'action=save'),array('class'=>'form-horizontal')) ?>
...
<?php if(db_count($custom_module_table,'1','is_folder')): ?>
    <div class="form-group">
    	<label class="col-md-3 control-label" for="name"><?php echo TEXT_PARENT ?></label>
      <div class="col-md-9">	
    	  <?php echo select_tag('parent_id',custom_module::get_folder_choices(),$obj['parent_id'],array('class'=>'form-control input-large')) ?>        
      </div>			
    </div>
<?php endif; ?>        
    ...
    $('#configuration_form').validate({ignore:'',
        ...
        rules:{
            code:{
                ...
                remote:{ 
                    type: "POST",
                    url: url_for(plugin_name + '/custom_module/code','action=validate'),
                    data: {
            ...
            name: {
                remote:{
                    type: "POST",
                    url: url_for(plugin_name + '/custom_module/code','action=check_unique'),
                    data: {
                        name:  function () { 
                            return $("#name").val() 
                        },
